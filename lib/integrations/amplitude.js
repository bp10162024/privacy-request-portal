...
async function deleteUser(emailOrUserId) {
  ...
  for (const p of PROJECTS) {
    ...
    try {
      const res = await axios.post(
        ...
        {
          user_ids: [emailOrUserId],
          requester: 'privacy-request-portal',
      ...
      if (res.status >= 400) {
        ...
        results.push({ project: p.name, status: 'completed', request_id: res.data?.[0]?.request_id || res.data?.requests_received, raw: res.data });
      }
    } catch (err) {
  ...
  if (anyFailed && !anyOk) {
    ...
    status = 'completed';
    notes = 'Partial success — see breakdown';
  } else {
    status = 'completed';
  }
  return {
    ok: !anyFailed || anyOk,
    status,
    notes: notes || results.map(r => `${r.project}: ${r.status}${r.request_id ? ` (req ${r.request_id})` : ''}${r.error ? ` — ${JSON.stringify(r.error).slice(0, 120)}` : ''}`).join('; '),
