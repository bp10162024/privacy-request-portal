...
async function maybeAutoComplete(req, actorEmail) {
  ...
  // Build a per-destination outcome summary for the Slack channel notification
  const summaryLines = actions.map(a => {
    const status = a.status;
    let note = '';
    if (a.error_message) {
      note = ` _(${a.error_message.slice(0, 80)})_`;
    } else if (a.response_data && a.response_data.not_applicable_reason) {
      note = ` _(not applicable)_`;
    } else if (a.external_reference) {
      note = ` _(ref: ${String(a.external_reference).slice(0, 40)})_`;
    } else if (a.response_data && a.response_data.notes) {
      note = ` _(${String(a.response_data.notes).slice(0, 80)})_`;
    } else if (a.response_data && a.response_data.manual) {
      note = ` _(manually marked done)_`;
    }
    return `• *${a.destination}*: ${status}${note}`;
  }).join('\n');
  ...
  const lines = [
    ...
    `*Received:* ${new Date(r.date_received).toLocaleDateString()}`,
    `*Completed in:* ${Math.round((new Date(r.completed_at || Date.now()) - new Date(r.date_received)) / (1000 * 60 * 60 * 24))} days`,
    ``,
    `*Destination outcomes:*`,
    summaryLines,
    ``,
    `:envelope: *Action needed:* Reply to the requester via Intercom (or email if mail/phone source) confirming their request has been processed. The CCPA Request Runbook in \`~/Desktop/Claude Docs/\` has the completion macro.`,
    `:link: ${process.env.BASE_URL}/requests/${req.id}`,
  ];
